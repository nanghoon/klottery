    ...
    function html5Upload() {
    	...
    	sUploadURL  = '/klottery/admin/editorFileUpload.do';
    	//파일을 하나씩 보내고, 결과를 받음.
    	for(var j=0, k=0; j < nImageInfoCnt; j++) {
    		...
    		try{
	    		if(!!tempFile){
	    			...
	    			//callAjaxFileUpload(tempFile,sUploadURL);
	    			k += 1;
	    		}
	    	}catch(e){}
    		tempFile = null;
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
    	...
    	request.open('POST',sUploadURL);
    	var formData = new FormData();
    	formData.append("content-type" , "multipart/form-data");
    	formData.append("file" , tempFile);
    	request.send(formData);
    	request.onload = function(){
        	var sResString = request.responseText;
    ...
    function makeArrayFromString(sResString){
    	console.log("sResString" + sResString);
    	var	aTemp = [],
    		aSubTemp = [],
    		htTemp = {}
    		aResultleng = 0;
	...
	function onAjaxError (e){
		...
		console.log(e._response.responseText);
	}
 	...
 	function callFileUploader (){
 		console.log("callFileUplaoder()")
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			//sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/file_uploader.php',	//샘플 URL입니다.
 			//sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/file_uploader.jsp',	//샘플 URL입니다.
 			sUrl  : '/klottery/admin/editorFileUpload.do',	//샘플 URL입니다.
